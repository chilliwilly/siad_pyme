$(document).ready(function(){
	$("#in_plan_net").on('change',function(){
		if($("#in_plan_net :selected").text().toLowerCase().indexOf("telefonia") >= 0 && $("#in_plan_net :selected").text().toLowerCase().indexOf("tv") >= 0 && $("#in_plan_net :selected").text().toLowerCase().indexOf("banda ancha") >= 0){
			swal({
				...
				function(isConfirm) {
				 	if (isConfirm) {
						...
						$("#in_plan_net_adic").removeAttr('disabled');
						$("#in_plan_tv_pack").removeAttr('disabled');
						$("#btnAgregaDeco").removeAttr('disabled');
						$("#in_deco_basico").removeAttr('disabled');

						$("#in_plan_fono_adic").attr('disabled','disabled');
						$("#in_plan_fono_adict").attr('disabled','disabled');
				    	...
				    	$("#in_plan_fono_adic").removeAttr('disabled');
				    	$("#in_plan_fono_adict").removeAttr('disabled');
				    	$("#in_central_tf").selectpicker('refresh');
				  	}				  	
			});
		}
	    ...
	    if($("#in_plan_net :selected").text().toLowerCase().indexOf("telefonia") >= 0){
	      	swal({
				    title: "Central Telefonica",
				    text: "Lleva central telefonica?",
				    type: "warning",
